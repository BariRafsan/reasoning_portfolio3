...
def play_player_hands(
    player_cards, dealer_upcard_value, shoe, agent, rules, get_tc_bucket
):
    ...
    while pending:
        ...
        while True:
            ...
            can_double = (
                h["first_action"]
                and len(cards) == 2
                and (not h["from_split"] or rules.double_after_split)
            )
            can_split = (
                h["first_action"]
                and is_pair
                and len(cards) == 2
                and num_hands_active < rules.max_hands
            )
            can_surrender = (
                h["first_action"]
                and not h["from_split"]
                and len(cards) == 2
                and rules.surrender_allowed
            )
...
def play_round(shoe, rules, agent, get_tc_bucket_fn=None):
    ...
    shoe.reveal(player_cards[0])
    shoe.reveal(player_cards[1])
    ...
    hand_results = play_player_hands(
        player_cards, dealer_upcard_value, shoe, agent, rules, get_tc_bucket
    )
